...
GET RID OF SILLY QSTRING STUFF
...
<?} else {?>
<?if ($mode == 0) {?>
<TD BGCOLOR="#F1EFD7">Subject: <INPUT TYPE="text" NAME="subject" SIZE="30" MAXLENGTH="40" CLASS="formtext"></TD>
<?} else {?>
...
$forumrs = mysql_query('SELECT forum_id, forum_name, forum_topics, forum_posts, forum_lastpost, poster, topic, topic_title, post_time, Username FROM twhl_forums LEFT JOIN twhl_forumposts ON forum_lastpost = post_id LEFT JOIN twhl_forumtopics ON topic = topic_id LEFT JOIN twhl_users ON poster = UserID',$GLOBALS['dbh']) or die('Database error');
if (mysql_num_rows($forumrs) > 0) {
  ...
  while ($frow = mysql_fetch_array($forumrs)) {
    ...
    ?>
<TR><TD <?=($cnt)?'BGCOLOR="#F8F6DE"':''?>><?=($newpost)?'<FONT COLOR="red"><B>*</B></FONT> ':''?><A HREF="<?=SELF.qstr('id',$frow['forum_id'],qstr('action','viewforum',qstr('pgt','1','')))?>"><?=$frow['forum_name']?></A></TD>
...
$s_forumrs = mysql_query('SELECT forum_name, forum_topics FROM twhl_forums WHERE forum_id = '.$id,$GLOBALS['dbh']) or die('Database error');
...
$topicrs = mysql_query('SELECT topic_id, topic_title, topic_poster, topic_time, topic_views, topic_replies, post_time, twhl_users.Username, twhl_users2.Username AS lastpost_uname FROM twhl_forumtopics LEFT JOIN twhl_forumposts ON topic_lastpost = post_id LEFT JOIN twhl_users ON topic_poster = twhl_users.UserID LEFT JOIN twhl_users AS twhl_users2 ON poster = twhl_users2.UserID WHERE forum = '.$id.' ORDER BY post_time DESC LIMIT '.getpgoffset(1).','.getpgoffset(0),$GLOBALS['dbh']) or die('Database error');
...
<?=(MOD_POST)?'<TD '.(($cnt)?'BGCOLOR="#F8F6DE"':'').' STYLE="font-size: 7pt;" ALIGN="right">[<A HREF="'.SELF.'action=a_edittopic&id='.$trow['topic_id'].'">edit</A>] [<A HREF="'.SELF.'action=a_deltopic&id='.$trow['topic_id'].'">del</A>]</TD>':''?>
</TR>
    <?
  }
}
...
$s_topicrs = mysql_query('SELECT forum, topic_title, forum_name, topic_replies, topic_open FROM twhl_forumtopics LEFT JOIN twhl_forums ON forum = forum_id WHERE topic_id = '.$id,$GLOBALS['dbh']) or die('Database error');
...
<?
$postrs = mysql_query('SELECT post_id, poster, post_text, post_time, Username, Avatar, ForumPosts FROM twhl_forumposts LEFT JOIN twhl_users ON poster = UserID WHERE topic = '.$id.' ORDER BY post_time ASC LIMIT '.getpgoffset(1).','.getpgoffset(0),$GLOBALS['dbh']) or die('Database error');
...
<?=post_format($prow['post_text'])?>
</TD><TD BGCOLOR="#F1EFD7" WIDTH="1"></TD>
<TD WIDTH="180" <?=($cnt)?'BGCOLOR="#F8F6DE"':''?> STYLE="font-size: 8pt;" VALIGN="top">
<?=(!$first)?'<HR COLOR="#cbc9b2" NOSHADE WIDTH="100%" SIZE="1">':''?>
Posted on <?=date('d M y H:i',getztime($_SESSION['tz'],$prow['post_time']))?><BR>
by <?=getuname($prow['Username'])?> 
<?=(MOD_POST)?'[<A HREF="'.SELF.'action=a_editpost&id='.$prow['post_id'].'">edit</A>] [<A HREF="'.SELF.'action=a_delpost&id='.$prow['post_id'].'">del</A>]':''?>
<?if (!isset($_SESSION['forumimages']) || $_SESSION['forumimages']) {?><DIV ALIGN="center"><?if ($prow["Avatar"] > 1) {?><IMG SRC="gfx/avatars/<?=str_pad($prow["Avatar"],3,"0",STR_PAD_LEFT)?>.gif" ALT="Avatar"><?}elseif($prow["Avatar"] == -1) {?><IMG SRC="avatars/<?=$prow['poster']?>.jpg" ALT="Avatar"><?}elseif($prow["Avatar"] == -2) {?><IMG SRC="avatars/<?=$prow['poster']?>.png" ALT="Avatar"><?}?></DIV><?}?>
</TD></TR>
...
<P>
<A NAME="#reply"></A>
<?if ($s_trow['topic_open']) echo postform(1); else echo '<center><i>This topic has been closed by a moderator</i></center>';?>
<?
}
...
function create_topic($id) {
  ...
  mysql_query('INSERT INTO twhl_forumtopics (forum, topic_title, topic_poster, topic_time) VALUES ('.$id.', "'.$subject.'", '.$user.', '.$ttime.')',$GLOBALS['dbh']) or die('Database error');
