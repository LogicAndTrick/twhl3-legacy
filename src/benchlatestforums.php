...
postID, threads.name, posts.threadid, posts.forumcat, posterid, postdate, postedit, posttext, uid, userID, accesslevel
FROM posts
...
$bench['inc_cent_forums_query'] = bcsub($endtime,$starttime,6);
$starttime = $endtime;
//------------------------------------------//
...
$timeparts = explode(" ",microtime());
$endtime = $timeparts[1].substr($timeparts[0],1);
$bench['inc_cent_forums_post_'.$row['postID']] = bcsub($endtime,$starttime,6);
$starttime = $endtime;
//------------------------------------------//
	}
